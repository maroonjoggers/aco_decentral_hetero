...
def main():
    ...
    while True:
        ...
        if not WITH_LAMBDA:
            agent_velocities_si = network_barriers_with_obstacles_safe(agent_velocities_si_nominal, x[:2], env, None, False)
        else:
            lambda_values = np.array([
                controller.rl_agents[i].select_lambda(current_time)
                for i in range(NUM_AGENTS)
            ])
            agent_velocities_si = network_barriers_with_obstacles_safe(agent_velocities_si_nominal, x[:2], env, lambda_values, True)
